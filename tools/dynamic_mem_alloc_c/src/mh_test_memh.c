...
#define MEMH_TEST_BLOCK_SIZE	(1 << 20)
#define MEMH_DELAY_UNIT			(1)

/** Enable this flag during comiplation and you will have two threads:
 * One to allocate and free the memeory, and the other to print the stats
 * every 5 seconds.
 */
...
int main(int argc, char** argv) {
	...
	/** TODO: change the seed everytime with srand(time(NULL)). */
	unsigned int seed = 0xFEABDEED;
	srand(seed);
#ifdef MEMH_ALLOW_THREAD_TO_PRINT_STATS
	pthread_t thID1;
	...
	fprintf(stderr, "\t\tUsage: <prog_name> <num_mem_blocks>,  "
			"where num_mem_blocks:[0-99]\n");
	for (int i = 0; i < size; i++) {
		fprintf(stderr, "Allocating one block %d\n", i);
		int size = (rand() % (MEMH_TEST_BLOCK_SIZE)) + 1;
		/** size = (size == 0) ? 1 : size; */
